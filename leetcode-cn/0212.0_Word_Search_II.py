...
class Solution:
    def findWords(self, board: List[List[str]], words: List[str]) -> List[str]:
        ...
        def check(w, i, x, y, seen):
            if i == len(w) - 1:
                return True
            for dx, dy in dirs:
                nx, ny = x + dx, y + dy
                idx = nx * N + ny
                if 0 <= nx < M and 0 <= ny < N and idx not in seen and board[nx][ny] == w[i+1]:
                    seen.add(idx)
                    if check(w, i + 1, nx, ny, seen):
                        return True
                    seen.discard(idx)
...
'''
最后一个abab那个盘，剪枝一下，记录下盘上出现过的所有字母，如果单词里某个字母没有在盘上出现直接continue

执行用时：448 ms, 在所有 Python3 提交中击败了63.40% 的用户
内存消耗：15.1 MB, 在所有 Python3 提交中击败了95.08% 的用户
通过测试用例：42 / 42
'''
class Solution:
    def findWords(self, board: List[List[str]], words: List[str]) -> List[str]:
